...
2. Добавить `.gitignore`. Содержание файла можно взять с ресурса: https://github.com/github/gitignore/blob/master/Swift.gitignore. После добавления необходимо раскомментировать `Pods`.
3. Создать новый swift-проект (Single View App). Переключиться на новую ветку `dev`. Cделать коммит и запушить изменения на remote-сервер в `dev` ветку
4. Установить на рабочем компьютер гем [cocoapods](https://cocoapods.org/), исходя из инструкции на официальном сайте
5. Интегрировать библиотеку [Alamofire](https://github.com/Alamofire/Alamofire) в проект с помощью Cocoapods, запусить созданный автоматически файл `.xcworkspace`, запустить проект на симуляторе
